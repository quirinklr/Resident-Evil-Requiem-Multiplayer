...
$autorunModuleDir = Join-Path $autorunDir "re9mp"
$dataDir = Join-Path $game "reframework\data\re9mp"
New-Item -ItemType Directory -Force -Path $pluginsDir, $autorunDir, $autorunModuleDir, $dataDir | Out-Null
...
Copy-Item -Path (Join-Path $root "reframework\autorun\re9mp\*") -Destination $autorunModuleDir -Recurse -Force

Write-Host "Deployed re9mp.dll, re9mp.lua, and Lua modules to $game"
